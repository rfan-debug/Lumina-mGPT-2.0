...
class ChameleonImageVocabularyMapping:
    ...
    def get_image_tokens(self):
        return sorted([val for name, val in self.vocab_map.items() if name.startswith("<MOVQGAN")])
...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if hasattr(self.config, "quantization_config"):
            ...
            self.use_quant_infer=True
        else:
            self.use_quant_infer=False
        # print("*"*100000, self.use_quant_infer)
    ...
    def forward(
        ...
        ```"""
        if not self.use_quant_infer:
            return self.forward_decoding(input_ids, pixel_values, attention_mask, position_ids, past_key_values, inputs_embeds,\
                                        labels, use_cache, output_attentions, output_hidden_states, return_dict, cache_position)
        else:
            # print("Yes")
            if input_ids.shape[1] > 1 and self.forward_call_times==0:
                # from pdb import set_trace; set_trace()
                self.forward_call_times+=1
                return self.forward_prefill(input_ids, pixel_values, attention_mask, position_ids, past_key_values, inputs_embeds,\
                                            labels, use_cache, output_attentions, output_hidden_states, return_dict, cache_position)
            else:
                return self.forward_decoding(input_ids, pixel_values, attention_mask, position_ids, past_key_values, inputs_embeds,\
                                            labels, use_cache, output_attentions, output_hidden_states, return_dict, cache_position)
