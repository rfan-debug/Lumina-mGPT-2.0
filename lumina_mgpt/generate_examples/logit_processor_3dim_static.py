"""
Reference:
    https://github.com/tyshiwo1/Accelerating-T2I-AR-with-SJD/blob/main/scheduler/logit_processor_3dim.py
"""
...
def get_eol_in_multitokens(logits, eol_cls, tokenlen, new_pred_tokenlen, line_len, min_dtype=-math.inf):
    ...
    # print(logits_forced_eol.shape, eol_position_ids)
...
class MultiTokensVLLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # print('self.image_next_line_token_id', self.image_next_line_token_id, 'input_ids', input_ids, input_ids.shape, 'scores', scores, scores.shape) # self.image_next_line_token_id: 8803 ; input_ids.shape: torch.Size([1, 88]) ; scores.shape: shape len 2 -> torch.Size([1, 1, 65536])
        ...
        if self.num_image_start_tokens == self.num_image_end_tokens:
            ...
            if new_token_num >= 2:
                ...
                if self.h_latent_dim is None or self.w_latent_dim is None:
                    ...
                    assert self.h_latent_dim > 0 and self.w_latent_dim > 0, f"Error: h_latent_dim {self.h_latent_dim} w_latent_dim {self.w_latent_dim}"
...
class MultiTokensInterleavedTopKLogitsWarper(LogitsProcessor):
    r"""
...
class TopnsigmaLogitsWarper3d(LogitsProcessor):

    def __init__(self, n: int = 1, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1, ):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        logits = scores
        n = self.n
        # logits_max, logits_max_ids = logits.max(dim=-1, keepdim=True)
        # thres = logits_max - n*logits.std(dim=-1, keepdim=True)
        # mask = (logits < thres)
        # mask.scatter_(-1, logits_max_ids, False)
        mask = get_topnsigma_mask(logits, n=n, logit_min_val=-10000)
        scores_processed = scores.masked_fill(mask, self.filter_value)

        return scores_processed

def get_statistics_with_invalid_values(inputs, min_val=-float("Inf"), max_val=float("Inf"), dim=-1):
    valid_logits_mask = (inputs > min_val) & (inputs < max_val)
        
    valid_logits_num_ratios = inputs.shape[dim] / valid_logits_mask.float().sum(dim=dim, keepdim=True).clamp(min=1)
    valid_logits = inputs.clone()
    valid_logits[~valid_logits_mask] = 0
    valid_logits_mean = valid_logits.mean(dim=dim, keepdim=True) * valid_logits_num_ratios
    nonelem_valid_logits_mean = valid_logits_mean.clone()
    valid_logits_mean = valid_logits_mean.expand_as(inputs)
    valid_logits_mean[~valid_logits_mask] = 0
    valid_logits_std = ((valid_logits - valid_logits_mean).pow(2).mean(dim=dim, keepdim=True) * valid_logits_num_ratios).sqrt()
    
    return nonelem_valid_logits_mean, valid_logits_std

def determine_in_topk(logits, token_indices, top_k=1):
    # torch.topk(logits, top_k)[0][..., -1, None]
    mask = torch.ones(logits.shape, dtype=torch.bool, device=logits.device)
    indices_to_remove = logits < torch.topk(logits, top_k)[0][..., -1, None]
    mask = mask.masked_fill(indices_to_remove, False)
    is_in_topk = mask.gather(-1, token_indices.unsqueeze(-1)).squeeze(-1)
    return is_in_topk

def get_topnsigma_mask(logits, n=1, logit_min_val=None):
    logits_max, logits_max_ids = logits.max(dim=-1, keepdim=True)

    if logit_min_val is not None:
        # indices_to_remove = scores < torch.topk(scores, top_k)[0][..., -1, None]
        _, sigma = get_statistics_with_invalid_values(logits, logit_min_val)
    else:
        sigma = logits.std(dim=-1, keepdim=True)

    thres = logits_max - n*sigma
    mask = (logits < thres)
    mask.scatter_(-1, logits_max_ids, False)
    # print(f"sigma {sigma.max()} logits_max {logits_max.max()} ")
    return mask
...
def sequential_calibration_sample(logits, generator=None):

    scores = nn.functional.softmax(logits, dim=-1)
    
    next_tokens, probs = multinomial_token_sample(logits[:, 0:1, :], generator=generator)
    ...
    for i in range(1, logits.shape[1]):
        ...
        next_tokens_i, probs_i = multinomial_token_sample(delta_logits, generator=generator)
        next_tokens.append(next_tokens_i)
...
def multinomial_token_sample(logits, generator=None, is_input_score=False):
    if is_input_score:
        probs = logits
    else:
